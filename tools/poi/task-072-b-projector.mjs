...
function resolveCachePath(value) {
  ...
  if (marker)
    candidates.push(`outputs/task-071-20260920/source-cache/${marker[1]}`);
...
function loadInput() {
  ...
  const candidates = new Map(
    candidateRows.map((row) => [row.candidateKey, row]),
  );
  ...
  for (const file of featureFiles) {
    ...
    for (const row of rows) {
      assert(
        !featureRows.has(row.candidateKey),
        `Duplicate feature row ${row.candidateKey}`,
      );
  ...
  assert(
    sourceBatches.length === 53,
    "Frozen phase manifest must contain 53 batches",
  );
  assert(
    frozenKeys.length === 10097,
    "Frozen phase manifest must contain 10097 candidates",
  );
  assert(
    new Set(frozenKeys).size === 10097,
    "Frozen phase manifest contains duplicates",
  );
  for (const key of frozenKeys)
    assert(candidates.has(key), `Missing candidate ${key}`);

  const evidence = new Map();
  const evidenceDir = pathFor(
    "data/poi/full/reviews/task-071/evidence-remediation",
  );
  for (const file of readdirSync(evidenceDir)
    .filter((name) => name.endsWith(".evidence-v2.jsonl"))
    .sort()) {
    for (const row of readJsonl(
      `data/poi/full/reviews/task-071/evidence-remediation/${file}`,
    )) {
      assert(
        !evidence.has(row.candidateKey),
        `Duplicate evidence row ${row.candidateKey}`,
      );
  ...
  for (const file of readdirSync(reviewDir)
    .filter((name) => name.endsWith(".discovery-v2.jsonl"))
    .sort()) {
    for (const row of readJsonl(`data/poi/full/reviews/task-071/${file}`))
      discovery.set(row.candidateKey, row);
  }
  ...
  return {
    ...
    evidence,
    discovery,
    observations: readJsonl(
      "data/poi/full/sources/identity-observations.v1.jsonl",
    ),
    articleIndex: readJson(
...
function observationsByCandidate(input) {
  ...
  for (const observation of input.observations) {
    if (!map.has(observation.sourceRecordId))
      map.set(observation.sourceRecordId, []);
...
function isAuthoritativeUrl(url) {
  return (
    /\.(go|lg)\.jp\//i.test(url) ||
    /japan\.travel/i.test(url) ||
    /nabunken\.go\.jp/i.test(url)
  );
}
...
function buildIdentityResolver(input) {
  ...
  return (candidateKey, phase) => {
    ...
    const candidateObservations =
      observations.get(candidateKey) ?? candidate.observations ?? [];
    const names = unique([
      ...(candidate.namesJa ?? []),
      ...(candidate.namesEn ?? []),
      ...candidateObservations.flatMap((o) => [
        o.nameJa,
        o.nameEn,
        ...(o.aliases ?? []),
      ]),
    ...
    const prefecture =
      ...
      candidateObservations.find((o) => o.prefecture)?.prefecture ??
      null;
    const municipality =
      candidateObservations.find((o) => o.municipality)?.municipality ?? null;
    const address =
      candidateObservations.find((o) => o.address)?.address ?? null;
    const coordinates =
      candidateObservations.find((o) => o.coordinates)?.coordinates ?? null;
    const category =
      candidateObservations.find((o) => o.entityType)?.entityType ?? null;
    const hasGeo = Boolean(
      municipality ||
      address ||
      coordinates ||
      retained.some((row) => row.prefectureMatchedInBody === true),
    );
    const authoritativeRetained = retained.filter(
      (row) =>
        row.acceptedForIdentity &&
        (row.sourceTier === "GOVERNMENT_OR_PUBLIC_BODY" ||
          isAuthoritativeUrl(row.url)),
    );
    const hasAuthoritative =
      authoritativeRetained.length > 0 || urls.some(isAuthoritativeUrl);
    const signalNames = [
      ...
      (discovery.openedSources ?? []).some((row) =>
        /instagram|facebook|x\.com|twitter|youtube/i.test(row.url),
      )
        ? "official_sns_candidate"
        : null,
    ].filter(Boolean);
    const competingTargets = retainedNames.filter(
      (name) => !nameSet.has(norm(name)),
    );
    const explicitConflict =
      /CONFLICT/i.test(evidence.identityDisposition ?? "") ||
      Boolean(evidence.identityHoldPreserved);
    ...
    if (phase === "A") {
      if (accepted.length && !materialConflict) {
        ...
        confidence = 0.92;
        rationale =
          "Retained government/public-body identity evidence matches the candidate name and frozen prefecture; no material competing target was retained.";
      } else if (materialConflict) {
        disposition = "IDENTITY_CONFLICT_HOLD";
        confidence = 0.25;
        rationale =
          "Retained or discovery evidence contains a material competing identity signal; enrichment is held to avoid contamination.";
        ...
        if (
          ...
          hasGeo &&
          (hasAuthoritative || hosts.length > 1 || category)
        ) {
          disposition = "RESOLVED_MEDIUM";
          confidence = 0.75;
          rationale =
            "Candidate name/alias and discriminative geographic or categorical signals converge with the retained/source search inventory without a credible competing target.";
        } else {
          disposition = "SECOND_PASS_REQUIRED";
          confidence = 0.45;
          rationale =
            "Search and source inventory were evaluated, but the available signals do not safely exceed the two-signal identity threshold.";
        }
      } else {
        disposition = "SECOND_PASS_REQUIRED";
        confidence = 0.4;
        rationale =
          "Search and source inventory were evaluated, but no safe target-specific identity match was established.";
      }
    } else if (phase === "B") {
      disposition = "IDENTITY_CONFLICT_HOLD";
      confidence = 0.2;
      rationale =
        "The frozen Phase B identity conflict remains held; TASK-072-B does not rebind Registry identity.";
    } else {
      disposition = "UNCHANGED_NON_PHASE_A";
      confidence = null;
      rationale =
        "Identity amendment applies to Phase A; the existing non-Phase-A pending population remains unchanged.";
    }
    return {
      candidateKey,
      phase,
      ...
      category,
      operator: null,
      officialDomains: hosts.filter((host) =>
        /\.go\.jp$|\.lg\.jp$|japan\.travel$|nabunken\.go\.jp$/i.test(host),
      ),
      officialSns: urls.filter((url) =>
        /instagram|facebook|x\.com|twitter|youtube/i.test(url),
      ),
      sourceRefs: urls,
      queriesAttempted: (discovery.queryFamilies ?? [])
        .map((row) => row.query)
        .filter(Boolean),
      sourcesOpened: (discovery.openedSources ?? []).map((row) => ({
        url: row.url,
        sourceTier: row.sourceTierCandidate,
        textSha256: row.fetch?.textSha256 ?? null,
      })),
...
function validateRetainedHashes(input) {
  ...
  assert(
    !failures.length,
    `Retained evidence hash validation failed: ${failures.slice(0, 5).join("; ")}`,
  );
...
function sourceEvidenceForCandidate(input, candidate, identity) {
  const byUrl = new Map(
    input.articleIndex.entries.map((row) => [row.url, row]),
  );
  const reviewedByRef = new Map(
    input.reviewed.entries.map((row) => [row.sourceRef, row]),
  );
  const matches = [];
  for (const url of candidate.evidenceRefs ?? []) {
    ...
    const articleNames = [article.nameJa, article.nameEn]
      .filter(Boolean)
      .map(norm);
  ...
  if (
    !matches.length ||
    !["RESOLVED_HIGH", "RESOLVED_MEDIUM"].includes(identity.disposition)
  )
    return [];
  return matches;
}
...
function validateReviewedFeature(feature, review, article) {
  ...
  assert(
    Number.isInteger(feature.value) && feature.value >= 0 && feature.value <= 9,
    `Invalid score ${feature.code}`,
  );
  assert(validateLocator(feature.locator), `Malformed locator ${feature.code}`);
  assert(
    feature.locator.offset + feature.locator.length <= article.targetContentEnd,
    `Locator outside target ${feature.code}`,
  );
  assert(
    review.contentSha256 === article.contentSha256,
    `Reviewed content hash mismatch ${review.sourceRef}`,
  );
}
...
function buildFeatureDecisions(input, featureRow, candidate, identity) {
  ...
  for (const match of matches) {
    for (const feature of match.review.features ?? []) {
      ...
      if (!proposed.has(feature.code))
        proposed.set(feature.code, { feature, match });
    }
  }
  const decisions = [];
  for (const [code, key, kind] of FEATURE_CODES) {
    const currentValue = currentValues[code] ?? null;
    const preserved = existingProvenance.filter(
      (row) => row.featureCode === code,
    );
    const proposal = proposed.get(code);
    if (currentValue !== null) {
      const fact = preserved
        .flatMap((row) => row.facts ?? [])
        .find((item) => validateLocator(item.locator));
      decisions.push({
        featureCode: code,
        currentValue,
        ...
        confidence: preserved.length
          ? Math.min(...preserved.map((row) => row.confidence ?? 0))
          : null,
        rationale:
          "Existing supported candidate feature preserved by default; no stronger contradictory evidence was accepted.",
        rubricVersion: RUBRIC_VERSION,
        annotationMethod: "preserved_existing_provenance",
        locatorHash: fact?.locator ?? null,
        noEvidenceReason: fact
          ? null
          : "Existing non-null value has no machine-readable locator in the preserved provenance; value was not changed.",
        kind,
        key,
      });
      continue;
    ...
    if (proposal) {
      ...
      decisions.push({
        ...
        locatorHash: {
          sourceRef: match.review.sourceRef,
          contentSha256: match.review.contentSha256,
          ...feature.locator,
        },
  ...
  assert(
    decisions.length === 43,
    `${candidate.candidateKey} must have 43 feature decisions`,
  );
  for (const decision of decisions) {
    assert(
      ALLOWED_DISPOSITIONS.has(decision.disposition),
      `Invalid disposition ${decision.disposition}`,
    );
    assert(
      decision.rubricVersion === RUBRIC_VERSION,
      `Wrong rubric ${decision.featureCode}`,
    );
    assert(
      decision.proposedValue === null ||
        (Number.isInteger(decision.proposedValue) &&
          decision.proposedValue >= 0 &&
          decision.proposedValue <= 9),
      `Invalid proposed value ${decision.featureCode}`,
    );
    assert(
      decision.locatorHash || decision.noEvidenceReason,
      `Missing locator/no-evidence reason ${decision.featureCode}`,
    );
  }
...
function buildVisitAccess(candidate, identity, matches) {
  const visits = matches.flatMap(({ review, article }) =>
    review.visit ? [{ review, article, visit: review.visit }] : [],
  );
  const anchors = matches.flatMap(({ review, article }) =>
    (review.anchors ?? []).map((anchor) => ({ review, article, anchor })),
  );
  const visit =
    visits.length &&
    ["RESOLVED_HIGH", "RESOLVED_MEDIUM"].includes(identity.disposition)
      ? {
          ...
          profile: {
            ...
            recommendedDurationMinutes:
              visits[0].visit.recommendedDurationMinutes,
          ...
          locatorHashes: visits.map(({ review, visit }) => ({
      ...
      : {
          ...
          evidenceRefs: [],
          locatorHashes: [],
          noSupportedFactReason:
            identity.disposition === "IDENTITY_CONFLICT_HOLD" ||
            identity.disposition === "SECOND_PASS_REQUIRED"
...
function buildCandidateProjection(input, batch, key, resolveIdentity) {
  ...
  const { decisions, matches } = buildFeatureDecisions(
    input,
    featureRow,
    candidate,
    identity,
...
function metricsFor(batch, projections, identityRows, validatedRetained) {
  ...
  const newNonNull = decisions.filter(
    (row) => row.disposition === "ADD_SUPPORTED",
  );
  const superseded = decisions.filter(
    (row) => row.disposition === "SUPERSEDE_SUPPORTED",
  );
  const preserved = decisions.filter(
    (row) => row.disposition === "PRESERVE_SUPPORTED",
  );
  const identityCounts = Object.fromEntries(
    unique(identityRows.map((row) => row.disposition)).map((key) => [
      key,
      identityRows.filter((row) => row.disposition === key).length,
    ]),
  );
  return {
    phase: batch.phase,
    ...
    retainedTextCount: projections.reduce(
      (sum, row) => sum + row.retainedEvidenceRecordCount,
      0,
    ),
    featureExtractionAttemptedCount: projections.length,
    featureDecisionCount: decisions.length,
    candidatesWithAtLeastOneSupportedFeature: projections.filter((row) =>
      row.featureDecisions.some((decision) => decision.proposedValue !== null),
    ).length,
    ...
    identityDecisionAttemptedCount:
      batch.phase === "A" || batch.phase === "B" ? projections.length : 0,
    identityDispositionUpdatedCount:
      batch.phase === "A" ? projections.length : 0,
    visitExtractionAttemptedCount: projections.filter(
      (row) => row.visitExtraction.attempted,
    ).length,
    visitProfileAddedCount: projections.filter(
      (row) => row.visitExtraction.status === "ADDED",
    ).length,
    accessExtractionAttemptedCount: projections.filter(
      (row) => row.accessExtraction.attempted,
    ).length,
    accessAnchorAddedCount: projections.reduce(
      (sum, row) => sum + row.accessExtraction.anchors.length,
      0,
    ),
    ...
    supplementalOfficialSourceCount: identityRows.reduce(
      (sum, row) =>
        sum +
        (row.retained ?? row.retainedSourceSummaries ?? []).filter((e) =>
          isAuthoritativeUrl(e.url),
        ).length,
      0,
    ),
    supplementalOfficialSNSCount: identityRows.reduce(
      (sum, row) => sum + row.officialSns.length,
      0,
    ),
    unsupportedNullDecisionCount: decisions.filter(
      (row) => row.disposition === "UNSUPPORTED_REMAINS_NULL",
    ).length,
    contradictorySourceCount: identityRows.reduce(
      (sum, row) => sum + row.contradictions.length,
      0,
    ),
    reviewErrorQueueCount: identityRows.filter((row) =>
    ...
    identitySignalsEvaluatedCount: identityRows.reduce(
      (sum, row) => sum + row.signalsEvaluatedCount,
      0,
    ),
    competingTargetsDetectedCount: identityRows.reduce(
      (sum, row) => sum + row.competingTargetsDetectedCount,
      0,
    ),
    candidatesResolvedThenEnrichedCount: identityRows.filter((row) =>
      ["RESOLVED_HIGH", "RESOLVED_MEDIUM"].includes(row.disposition),
    ).length,
  };
}
...
function updateFeatureSidecars(input, projections, batch) {
  ...
  for (const projection of projections) {
    ...
    for (const decision of projection.featureDecisions) {
      if (
        decision.disposition === "ADD_SUPPORTED" ||
        decision.disposition === "SUPERSEDE_SUPPORTED"
      ) {
        sidecarChanged = true;
        ...
        row.provenance = [
          ...(row.provenance ?? []),
          {
            ...
            reasonCodes: [
              "TASK_072_B_PROJECTED",
              "RETAINED_TARGET_SPECIFIC_FACT",
            ],
            facts: [
              {
                sourceRef: decision.locatorHash.sourceRef,
                reason: decision.rationale,
                locator: decision.locatorHash,
              },
            ],
          },
        ];
  ...
  for (const file of touched)
    atomicWrite(
      `data/poi/full/features/${file}`,
      jsonl(input.featureRowsByFile.get(file)),
    );
  return [...touched].sort();
}

function validatePreflight(input, resolveIdentity) {
  const registryBefore = sha256File(
    "data/poi/full/registry/combined-candidates.v1.jsonl",
  );
  const masterBefore = sha256File(
    "src/shared/data/master-code-registry.v1.json",
  );
  const candidateIdentityBefore = sha256(
    json(
      input.candidateRows.map((row) => ({
  ...
  assert(
    input.rubric.rubricVersion === RUBRIC_VERSION,
    "rubricVersion exact match failed",
  );
  assert(
    input.rubric.definitions.length === 43,
    "rubric definition count failed",
  );
  assert(
    JSON.stringify(
      input.rubric.definitions.map(({ code, key, kind }) => [code, key, kind]),
    ) === JSON.stringify(FEATURE_CODES),
    "rubric feature contract mismatch",
  );
  const retainedHashValidation = validateRetainedHashes(input);
  const positive = [...input.featureRows.values()]
    .map(({ row }) => row)
    .filter((row) => {
      ...
      return (
        ...
        (row.provenance ?? []).every(
          (p) =>
            p.sourceRefs?.length &&
            p.facts?.some((fact) => validateLocator(fact.locator)),
        )
      );
    })
    .slice(0, 20);
  assert(positive.length >= 20, "Need at least 20 positive controls");
  const retainedControls = input.frozenKeys
    .filter(
      (key) => (input.evidence.get(key)?.retainedEvidence ?? []).length > 0,
    )
    .slice(0, 20);
  assert(
    retainedControls.length >= 20,
    "Need at least 20 retained-text controls",
  );
  const controlKeys = unique([
    ...positive.map((row) => row.candidateKey),
    ...retainedControls,
  ]);
  const plans = () =>
    controlKeys.map((key) => {
      const batch = input.sourceBatches.find((b) =>
        b.candidateKeys.includes(key),
      );
      const identity = resolveIdentity(key, batch?.phase ?? "A");
      const projection = buildCandidateProjection(
        input,
        batch ?? { batchId: "PREFLIGHT", phase: "A" },
        key,
        resolveIdentity,
      );
      return {
        candidateKey: key,
        featureDecisionCount: projection.featureDecisionCount,
        decisions: projection.featureDecisions,
        identityDisposition: identity.disposition,
      };
    });
  const first = plans();
  const second = plans();
  assert(
    first.length === controlKeys.length &&
      first.every((row) => row.featureDecisionCount === 43),
    "Preflight 43-decision shape failed",
  );
  assert(
    first.reduce((sum, row) => sum + row.featureDecisionCount, 0) ===
      controlKeys.length * 43,
    "Preflight decision total failed",
  );
  assert(
    first.some((row) =>
      row.decisions.some(
    ...
    "Positive-control reproduction failed",
  );
  assert(
    sha256(json(first)) === sha256(json(second)),
    "Deterministic repeated dry-run failed",
  );
  assert(
    first.every((row) =>
      row.decisions.every((decision) =>
        ALLOWED_DISPOSITIONS.has(decision.disposition),
      ),
    ),
    "Preflight schema validation failed",
  );
  assert(
    sha256File("data/poi/full/registry/combined-candidates.v1.jsonl") ===
      registryBefore,
    "Registry changed during preflight",
  );
  assert(
    sha256File("src/shared/data/master-code-registry.v1.json") === masterBefore,
    "Master Code changed during preflight",
  );
  const candidateIdentityAfter = sha256(
    json(
      input.candidateRows.map((row) => ({
        candidateKey: row.candidateKey,
        canonicalMasterCode: row.canonicalMasterCode,
  ...
  assert(
    candidateIdentityAfter === candidateIdentityBefore,
    "candidateKey identity changed during preflight",
  );
  return {
    status: "PASS",
    ...
    featureDecisionCount: first.reduce(
      (sum, row) => sum + row.featureDecisionCount,
      0,
    ),
    ...
    controls: first.map((row) => ({
      candidateKey: row.candidateKey,
      featureDecisionCount: row.featureDecisionCount,
      identityDisposition: row.identityDisposition,
    })),
  };
}
...
function loadReceipts() {
  ...
  return readdirSync(dir)
    ...
    .map((name) => {
      ...
      receipt.identityRows = (receipt.identityRows ?? []).map(
        compactIdentityRow,
      );
      return receipt;
    });
...
function renderResult(input, preflight, receipts, status, blocker = null) {
  const totals = receipts.reduce((out, row) => {
    for (const [key, value] of Object.entries(row.telemetry ?? {}))
      if (typeof value === "number") out[key] = (out[key] ?? 0) + value;
    return out;
  }, {});
  const allRows = [...input.featureRows.values()].map(({ row }) => row);
  const valueCount = (row) =>
    Object.values(row.featureSet?.values ?? {}).filter(
      (value) => value !== null,
    ).length;
  const featureCounts = Object.fromEntries(
    CODES.map((code) => [
      code,
      allRows.filter((row) => row.featureSet?.values?.[code] !== null).length,
    ]),
  );
  const bands = Object.fromEntries(
    [1, 10, 20, 30, 43].map((n) => [
      `>=${n}`,
      allRows.filter((row) => valueCount(row) >= n).length,
    ]),
  );
  ...
  const dispositionCounts = Object.fromEntries(
    [
      ...
      key,
      phaseA.filter((row) => row.disposition === key).length,
    ]),
  );
  const batchTable = receipts
    ...
    .join("\n");
  const secondPassRows = identity.filter((row) =>
    ["SECOND_PASS_REQUIRED", "IDENTITY_CONFLICT_HOLD"].includes(
      row.disposition,
    ),
  );
  const reasonCounts = Object.fromEntries(
    unique(
      secondPassRows.map(
        (row) =>
          row.missingDiscriminativeSignal ?? "material identity conflict",
      ),
    ).map((reason) => [
      reason,
      secondPassRows.filter(
        (row) =>
          (row.missingDiscriminativeSignal ?? "material identity conflict") ===
          reason,
      ).length,
  ...
  const exactLocal =
    totals.featureDecisionCount === 434171 &&
    totals.featureExtractionAttemptedCount === 10097 &&
    totals.visitExtractionAttemptedCount === 10097 &&
    totals.accessExtractionAttemptedCount === 10097 &&
    phaseA.length === 6049 &&
    Object.values(dispositionCounts).reduce((a, b) => a + b, 0) === 6049;
...
function updateResult(input, preflight, receipts, status, blocker) {
  ...
  atomicWrite(
    ...
    renderResult(input, preflight, receipts, status, blocker),
  );
}

function writeSecondPass(input, identityRows) {
  const rows = identityRows.filter((row) =>
    ["SECOND_PASS_REQUIRED", "IDENTITY_CONFLICT_HOLD"].includes(
      row.disposition,
    ),
  );
  atomicWrite(
    ...
    jsonl(
      rows.map((row) => ({
        ...
        recommendedNextSourceOrAction:
          row.disposition === "IDENTITY_CONFLICT_HOLD"
            ? "manual conflict adjudication using target-owned/government source"
            : "open a target-owned or government/municipality source with matching address/coordinates",
...
function writeFinalManifest(input, preflight, receipts) {
  ...
  const outputFiles = [
    ...
    (file, index, list) =>
      list.indexOf(file) === index && existsSync(pathFor(file)),
  );
  const outputChecksums = Object.fromEntries(
    outputFiles.map((file) => [file, sha256File(file)]),
  );
  const manifest = {
    ...
    frozenBatches: 53,
    totalFeatureDecisions: receipts.reduce(
      (sum, row) => sum + row.telemetry.featureDecisionCount,
      0,
    ),
    featureExtractionAttemptedCount: receipts.reduce(
      (sum, row) => sum + row.telemetry.featureExtractionAttemptedCount,
      0,
    ),
    visitExtractionAttemptedCount: receipts.reduce(
      (sum, row) => sum + row.telemetry.visitExtractionAttemptedCount,
      0,
    ),
    accessExtractionAttemptedCount: receipts.reduce(
      (sum, row) => sum + row.telemetry.accessExtractionAttemptedCount,
      0,
    ),
...
function runFull(input, preflight, resolveIdentity, resume) {
  ...
  for (const batch of input.sourceBatches) {
    ...
    if (resume && prior) {
      const validOutputs = prior.outputs.every(
        (output) =>
          existsSync(pathFor(output.path)) &&
          sha256File(output.path) === output.sha256,
      );
      if (validOutputs) {
        receipts.push(prior);
        ...
        updateResult(
          ...
          receipts,
          receipts.length === 53
            ? "IN_PROGRESS — local batches complete; delivery gate pending"
            : "IN_PROGRESS",
          null,
        );
        continue;
    ...
    const projections = batch.candidateKeys.map((key) =>
      buildCandidateProjection(input, batch, key, resolveIdentity),
    );
    assert(
      projections.length === batch.count,
      `Batch membership mismatch ${batch.batchId}`,
    );
    assert(
      projections.every((row) => row.featureDecisionCount === 43),
      `43D decision gate failed ${batch.batchId}`,
    );
    const identityRows = batch.candidateKeys.map((key) =>
      resolveIdentity(key, batch.phase),
    );
    allIdentityRows.push(...identityRows);
    const validatedRetained = {
      recordCount: projections.reduce(
        (sum, row) => sum + row.retainedEvidenceRecordCount,
        0,
      ),
    };
    const telemetry = metricsFor(
      batch,
      projections,
      identityRows,
      validatedRetained,
    ...
    assert(
      telemetry.featureExtractionAttemptedCount === telemetry.candidateCount,
      `Extraction attempt gate failed ${batch.batchId}`,
    );
    assert(
      telemetry.featureDecisionCount === telemetry.candidateCount * 43,
      `Decision total gate failed ${batch.batchId}`,
    );
    ...
    const accessContent = jsonl(
      projections.map((row) => ({
        task: TASK,
        batchId: batch.batchId,
        candidateKey: row.candidateKey,
        ...row.accessExtraction,
      })),
    );
    const identityContent = jsonl(
      identityRows.map((row) => ({
        candidateKey: row.candidateKey,
        phase: row.phase,
        beforeDisposition:
          input.evidence.get(row.candidateKey)?.identityDisposition ??
          input.sourceBatches.find((b) =>
            b.candidateKeys.includes(row.candidateKey),
          )?.reasonCode,
        afterDisposition: row.disposition,
        confidence: row.confidence,
        rationale: row.rationale,
        sourceRefs: row.accepted.length
          ? row.accepted.map((e) => e.url)
          : row.sourceRefs,
    ...
    const outputs = [decisionPath, visitPath, accessPath, identityPath].map(
      (file) => ({ path: file, sha256: sha256File(file) }),
    );
    const receipt = {
      ...
      registryChecksumBefore: sha256File(
        "data/poi/full/registry/combined-candidates.v1.jsonl",
      ),
      registryChecksumAfter: sha256File(
        "data/poi/full/registry/combined-candidates.v1.jsonl",
      ),
      candidateIdentityChecksumBefore: sha256(
        json(
          input.candidateRows.map((row) => ({
            candidateKey: row.candidateKey,
            canonicalMasterCode: row.canonicalMasterCode,
      ...
      candidateIdentityChecksumAfter: sha256(
        json(
          input.candidateRows.map((row) => ({
            candidateKey: row.candidateKey,
            canonicalMasterCode: row.canonicalMasterCode,
      ...
      reasoningConfiguration:
        "rules-only; frozen-rubric; source-attributed; no-default-score",
      startedAt,
  ...
  assert(
    ...
    "Final extraction attempt count mismatch",
  );
  assert(
    receipts.reduce(
      (sum, row) => sum + row.telemetry.visitExtractionAttemptedCount,
    ...
    "Final Visit attempt count mismatch",
  );
  assert(
    receipts.reduce(
      (sum, row) => sum + row.telemetry.accessExtractionAttemptedCount,
    ...
    "Final Access attempt count mismatch",
  );
  ...
  const phaseACount = [
    ...
    (sum, disposition) =>
      sum + phaseA.filter((row) => row.disposition === disposition).length,
    0,
  );
  assert(
    phaseA.length === 6049 && phaseACount === 6049,
    "Phase A identity reconciliation mismatch",
  );
  updateResult(
    input,
    preflight,
    receipts,
    "BLOCKED / PARTIAL",
    "Local projector gates pass, but exact final-head GitHub Quality Gate and Draft PR delivery are not yet verified.",
  );
  return manifest;
}
...
function main() {
  ...
  console.log(
    json({
      ...
      manifest: `${TASK_ROOT}/final-manifest.json`,
      totalFeatureDecisions: manifest.totalFeatureDecisions,
    }),
  );
}
...
try {
  ...
  try {
    const input = loadInput();
    const preflight = existsSync(pathFor(`${QA_ROOT}/preflight.json`))
      ? readJson(`${QA_ROOT}/preflight.json`)
      : null;
...
function compactIdentityRow(identity) {
  ...
  return {
    ...
    competingTargetsDetectedCount: identity.competingTargetsDetectedCount,
    retainedSourceSummaries: retained.map((row) => ({
      url: row.url,
      sourceTier: row.sourceTier,
      contentSha256: row.contentSha256,
      locatorSha256: row.locatorSha256,
      acceptedForIdentity: row.acceptedForIdentity,
    })),
  };
}
