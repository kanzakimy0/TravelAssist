...
const valueCount = (row) =>
  Object.values(row.featureSet.values).filter((v) => v !== null).length;
const official = (url) =>
  /\.(go|lg)\.jp\b|japan\.travel|kanko|tourism|\.or\.jp\b|official/i.test(
    String(url ?? ""),
  );
const sourceTier = (url) =>
  /\.(go|lg)\.jp\b/i.test(String(url ?? ""))
...
function shaFile(rel) {
  return hash(readFileSync(p(rel)));
}
function sourceRef(source) {
  return (
    "remaining-source:" +
    String(source.contentSha256 ?? source.textSha256 ?? "").slice(0, 24)
  );
}
...
function loadIdentityRemediationEvidence() {
  const specs = [
    ...
    },
    {
      ...
      path:
        IDENTITY_REMEDIATION_ROOT +
        "/sources/japan-travel-minobusan-kuonji.html",
  ...
  const sources = specs
    .map((spec) => {
      ...
      return {
        url: spec.url,
        sourceRef:
          "task-073-b-remediation-source:" + contentSha256.slice(0, 24),
  ...
  return {
    ...
    officialSNS: {
      ...
      ownershipStatus: sources.some(
        (source) =>
          source.role === "official_operator_identity_and_sns_ownership" &&
          source.matchedNeedle === "https://www.instagram.com/minobusankuonji/",
      )
        ? "VERIFIED"
        : "UNVERIFIED",
      proofSourceRefs: sources
        .filter(
          (source) =>
            source.role === "official_operator_identity_and_sns_ownership",
        )
...
function namesFor(candidate) {
  const values = [
    candidate.nameJa,
    candidate.nameEn,
    candidate.canonicalNameJa,
    ...(candidate.namesJa ?? []),
    ...(candidate.namesEn ?? []),
    ...(candidate.aliases ?? []),
    ...(candidate.observations ?? []).flatMap((o) => [
      o.nameJa,
      o.nameEn,
      ...(o.aliases ?? []),
      o.historicalName,
...
function municipalityFor(candidate) {
  ...
  return (
    o.municipality ?? o.city ?? o.locality ?? candidate.municipality ?? null
  );
}
...
function locateSource(source) {
  ...
  if (source.contentPath && existsSync(source.contentPath))
    return source.contentPath;
  if (source.textPath) {
    const candidate = p(
      "outputs/task-071-20260920/source-cache/pages/" + source.textPath,
    );
...
function canonicalSource(input, key) {
  ...
  const length = Math.min(
    body.length - offset,
    Number(boundary.length ?? body.length),
  );
  return {
    candidateKey: key,
    ...
    boundary: {
      offset,
      length,
      locatorSha256: hash(body.slice(offset, offset + length)),
    },
...
function retainedSources(input, key) {
  ...
  return rows
    .map((source) => {
      ...
      const body = readFileSync(path, "utf8");
      assert(
        !source.contentSha256 || source.contentSha256 === hash(body),
        "retained source hash mismatch: " + key,
      );
      const boundary = source.targetContentBoundary ?? {
        start: 0,
        end: body.length,
      };
      return {
        ...
        body,
        scope: body.slice(
          Number(boundary.start ?? 0),
          Number(boundary.end ?? body.length),
        ),
        sourceRef: sourceRef(source),
        locator: {
          offset: Number(boundary.start ?? 0),
          length:
            Number(boundary.end ?? body.length) - Number(boundary.start ?? 0),
          locatorSha256:
            source.locatorSha256 ??
            hash(
              body.slice(
                Number(boundary.start ?? 0),
                Number(boundary.end ?? body.length),
...
function loadInput() {
  ...
  const candidateByKey = new Map(
    candidates.map((row) => [row.candidateKey, row]),
  );
  const identitySeed = new Map(
    readJsonl(
      "data/poi/full/task-072-b-correction-v2/identity-projection.jsonl",
    ).map((row) => [row.candidateKey, row]),
  );
  const semanticAnnotations = readJsonl(
    "data/poi/full/task-072-b-correction-v2/semantic-annotations.jsonl",
  );
  const evidence = loadEvidence();
  const phaseByKey = loadPhaseMap();
  const ledgerByKey = new Map(
    current.ledger.entries.map((row) => [row.candidateKey, row]),
  );
  const currentByKey = new Map(
    current.rows.map((row) => [row.candidateKey, row]),
  );
  const keys = current.pending.map((row) => row.candidateKey);
  const trackA = keys.filter((key) =>
    ["SECOND_PASS_REQUIRED", "IDENTITY_CONFLICT_HOLD"].includes(
      identitySeed.get(key)?.disposition,
    ),
  );
...
function identityDecision(input, key) {
  ...
  const exact = sources.filter((source) => {
    ...
    return target
      ? nameSet.has(norm(target))
      : names.some((name) => canonicalSource(input, key)?.scope.includes(name));
  });
  const prefecture = candidate.prefectures?.[0] ?? "";
  const municipality = municipalityFor(candidate);
  ...
  if (
    exact.some(
      (source) =>
        source.prefectureMatchedInBody ||
        (prefecture && source.scope.includes(prefecture)),
    )
  )
    signals.push({
      type: "prefecture",
  ...
  if (
    exact.some((source) => municipality && source.scope.includes(municipality))
  )
    signals.push({
      type: "municipality",
      value: municipality,
  ...
  if (
    ...
    signals.push({
      type: "coordinates_or_map_context",
      value: candidate.coordinates ?? [candidate.latitude, candidate.longitude],
      discriminative: true,
    });
  if (exact.some((source) => official(source.url)))
    signals.push({
      type: "official_domain",
      value: exact.find((source) => official(source.url))?.url,
      discriminative: true,
    });
  const competingTargets = unique(
    retained
      .map((source) => source.targetName)
      .filter((target) => target && !nameSet.has(norm(target))),
  );
  const discriminative =
    signals.find((signal) => signal.discriminative) ?? null;
  const strongOfficial = exact.some(
    (source) =>
      /\.(go|lg)\.jp\b/i.test(source.url ?? "") || official(source.url),
  );
  const valid =
    exact.length > 0 &&
  ...
  let disposition;
  if (phase === "B" || seed?.disposition === "IDENTITY_CONFLICT_HOLD")
    disposition = "IDENTITY_CONFLICT_HOLD";
  else if (phase === "A" && valid && strongOfficial && signals.length >= 3)
    disposition = "RESOLVED_HIGH";
  else if (phase === "A" && valid && signals.length >= 2)
  ...
  return {
    ...
    missingDiscriminativeEvidence:
      disposition === "DEEP_RESEARCH_REQUIRED"
        ? "official or government source matching name plus municipality, address, coordinates, or operator"
        : null,
    recommendedNextAction:
      disposition === "DEEP_RESEARCH_REQUIRED"
        ? "Search official operator, government, tourism/DMO and official SNS sources; verify locality or coordinates before enrichment."
        : null,
  };
}
...
function annotateFeatures(input, identityByKey, key) {
  ...
  for (const [code, terms] of FEATURE_RULES) {
    ...
    const term = terms.find((value) =>
      source.scope.toLowerCase().includes(value.toLowerCase()),
    );
    if (!term) continue;
    ...
    const fact = {
      ...
      reason:
        'Target-scoped retained source contains the explicit term "' +
        term +
        '"; this supports ' +
        meta.key +
        " only, without inferring nearby-attraction or current-service facts.",
...
function makeBatches(keys, prefix) {
  ...
  for (let i = 0; i < keys.length; i += BATCH_SIZE)
    batches.push({
      batchId: prefix + "-" + String(batches.length + 1).padStart(4, "0"),
      candidateKeys: keys.slice(i, i + BATCH_SIZE),
    });
  return batches;
}
...
function revalidatedLegacyFacts(input, key) {
  ...
  if (!row || !entry) return [];
  const output = [];
  for (const oldFact of input.semanticAnnotations.filter(
    (fact) => fact.candidateKey === key,
  )) {
    if (row.featureSet.values[oldFact.featureCode] !== null) continue;
    if (entry.sourceRef && entry.sourceRef !== oldFact.sourceRef) continue;
    const source = retainedSources(input, key).find(
      (candidate) =>
        candidate.url === oldFact.sourceUrl &&
        candidate.contentSha256 === oldFact.contentHash,
    );
    if (!source) continue;
    const offset = Number(oldFact.locator?.offset ?? -1);
    const length = Number(oldFact.locator?.length ?? 0);
    const excerpt =
      offset >= 0 ? source.body.slice(offset, offset + length) : "";
    if (
      !excerpt ||
      hash(excerpt) !== oldFact.locator?.locatorSha256 ||
      hash(source.body) !== oldFact.contentHash
    )
      continue;
    ...
    output.push({
      candidateKey: key,
      featureCode: oldFact.featureCode,
      key: meta.key,
      kind: meta.kind,
      suggestedValue: oldFact.suggestedValue,
      sourceRef: oldFact.sourceRef,
      sourceUrl: oldFact.sourceUrl,
      sourceTier: oldFact.sourceTier,
      confidence: oldFact.confidence,
      reason:
        "TASK-073-B revalidated a retained target-scoped TASK-072 semantic fact byte-for-byte; unsupported dimensions remain null.",
      rubricVersion: RUBRIC_VERSION,
      annotationMethod: "task_073_b_revalidated_prior_annotation_v1",
      contentHash: oldFact.contentHash,
      locator: oldFact.locator,
    });
  }
  return output;
}
function buildAll(input) {
  ...
  for (const key of input.trackA)
    identityByKey.set(key, identityDecision(input, key));
  for (const key of input.trackB) {
    ...
    identityByKey.set(key, {
      ...
      rationale:
        "Upstream authoritative identity gate is outside the deep-resolution population and remains unchanged.",
    });
  }
  ...
  for (const key of input.keys) {
    ...
    const facts = [
      ...new Map(
        [...generated, ...legacy].map((fact) => [fact.featureCode, fact]),
      ).values(),
    ];
...
function decisionRows(input, identityByKey, factsByKey, key) {
  ...
  const identity = identityByKey.get(key);
  const facts = new Map(
    (factsByKey.get(key) ?? []).map((fact) => [fact.featureCode, fact]),
  );
  const decisions = [];
  for (const [code, featureKey, kind] of FEATURE_CODES) {
    const current = row.featureSet.values[code] ?? null;
    const previous = (row.provenance ?? []).filter(
      (fact) => fact.featureCode === code,
    );
    if (current !== null) {
      decisions.push({
        featureCode: code,
        ...
        sourceRefs: unique(previous.flatMap((fact) => fact.sourceRefs ?? [])),
        sourceTier: "existing_authoritative_current_view",
        confidence: previous[0]?.confidence ?? null,
        rationale:
          "Existing authoritative value loaded and preserved unchanged.",
        rubricVersion: RUBRIC_VERSION,
        annotationMethod: "preserve_authoritative_current_value",
    ...
    if (fact) {
      decisions.push({
        ...
        locatorHash: fact.locator,
        noEvidenceReason: null,
      });
      continue;
    }
    const blocked =
      identity.disposition === "DEEP_RESEARCH_REQUIRED" ||
      identity.disposition === "IDENTITY_CONFLICT_HOLD";
    decisions.push({
      featureCode: code,
      key: featureKey,
      kind,
      ...
      confidence: null,
      rationale: blocked
        ? "Identity disposition blocks safe enrichment; the field remains null."
        : "Target-scoped semantic annotation was attempted; no rubric-supported fact was retained.",
      rubricVersion: RUBRIC_VERSION,
      annotationMethod: "task_073_b_null_targeted_semantic_annotation",
      locatorHash: null,
      noEvidenceReason: blocked
        ? "identity deep research or conflict hold remains"
        : "no target-scoped supported evidence",
    });
  }
...
function projection(input, identityByKey, factsByKey, batch, track) {
  const projections = batch.candidateKeys.map((key) => {
    ...
    const row = input.currentByKey.get(key);
    const add = decisions.filter(
      (decision) => decision.disposition === "ADD_SUPPORTED",
    );
    return {
      ...
      semanticAnnotationAttempted:
        track === "B" || identity.resolvedForEnrichment,
      searchTrace: {
        ...
        targetScopedTextReadCount: identity.sourcesOpened.filter(
          (source) => source.targetScopeRead,
        ).length,
      },
      visitExtraction: {
        attempted: true,
        status: (row.visitProfiles ?? []).length
          ? "PRESERVED_EXISTING"
          : "NO_SUPPORTED_FACT",
        existingCount: (row.visitProfiles ?? []).length,
        addedCount: 0,
        noSupportedFactReason: (row.visitProfiles ?? []).length
          ? null
          : "No new target-scoped duration fact accepted in this pass.",
      },
      accessExtraction: {
        attempted: true,
        status: (row.accessLinks ?? []).length
          ? "PRESERVED_EXISTING"
          : "NO_SUPPORTED_FACT",
        existingCount: (row.accessLinks ?? []).length,
        addedCount: 0,
        noSupportedFactReason: (row.accessLinks ?? []).length
          ? null
  ...
  const identityRows = projections.map((row) =>
    identityByKey.get(row.candidateKey),
  );
  const facts = decisions.filter(
    (decision) => decision.disposition === "ADD_SUPPORTED",
  );
  const existing = decisions.filter(
    (decision) => decision.disposition === "PRESERVE_SUPPORTED",
  );
  const sources = projections.flatMap((row) => row.searchTrace.sourcesOpened);
  const telemetry = {
    track,
    batchId: batch.batchId,
    model: "task-073-b-retained-source-projector-v1",
    reasoningConfiguration: "target-scoped-semantic-evidence-v1",
    candidateCount: projections.length,
    queryCount: projections.reduce(
      (n, row) => n + row.searchTrace.queries.length,
      0,
    ),
    officialSitePages: sources.filter(
      (source) => sourceTier(source.url) === "OFFICIAL_SITE_OR_TOURISM",
    ).length,
    governmentTourismPages: sources.filter(
      (source) => sourceTier(source.url) === "GOVERNMENT_OR_PUBLIC_BODY",
    ).length,
    officialSNSAccounts: projections.filter((row) =>
      row.candidateKey.includes("SNS"),
    ).length,
    officialSNSPosts: 0,
    authoritativeSecondaryPages: sources.filter(
      (source) => sourceTier(source.url) === "AUTHORITATIVE_SECONDARY",
    ).length,
    retainedTextCount: sources.filter((source) => source.targetScopeRead)
      .length,
    semanticAnnotationAttemptedCount: projections.filter(
      (row) => row.semanticAnnotationAttempted,
    ).length,
    identityResolvedHighCount: identityRows.filter(
      (row) => row.disposition === "RESOLVED_HIGH",
    ).length,
    identityResolvedMediumCount: identityRows.filter(
      (row) => row.disposition === "RESOLVED_MEDIUM",
    ).length,
    deepResearchRequiredCount: identityRows.filter(
      (row) => row.disposition === "DEEP_RESEARCH_REQUIRED",
    ).length,
    identityConflictHoldCount: identityRows.filter(
      (row) => row.disposition === "IDENTITY_CONFLICT_HOLD",
    ).length,
    candidatesResolvedThenEnriched: projections.filter(
      (row) => row.resolvedThenEnriched,
    ).length,
    ...
    visitAttempted: projections.filter((row) => row.visitExtraction.attempted)
      .length,
    visitAdded: 0,
    accessAttempted: projections.filter((row) => row.accessExtraction.attempted)
      .length,
    accessAdded: 0,
    remainingNullDecisionCount: decisions.filter(
      (decision) => decision.proposedValue === null,
    ).length,
    rejectedEvidenceCount: projections.reduce(
      (n, row) =>
        n + (row.identityDisposition === "DEEP_RESEARCH_REQUIRED" ? 1 : 0),
      0,
    ),
...
function selectIdentityCanary(input, identityByKey) {
  ...
  const by = (predicate) =>
    rich.find((key) => predicate(identityByKey.get(key), key));
  add(
    by(
      (row, key) =>
        identityByKey.get(key)?.disposition === "IDENTITY_CONFLICT_HOLD",
    ),
  );
  add(by((row, key) => key.includes("SNS")));
  add(
    by((row) =>
      row.sourcesOpened.some(
        (source) => source.sourceTier !== "AUTHORITATIVE_SECONDARY",
      ),
    ),
  );
  ...
  assert(
    selected.length >= 30,
    "Identity canary requires 30 opened-source candidates",
  );
...
function identityCanary(input, identityByKey, factsByKey) {
  ...
  const traces = keys.map((key, index) => {
    ...
    if (
      row.names.length > 1 ||
      (input.names.has(norm(row.names[0])) &&
        input.names.get(norm(row.names[0])).length > 1)
    )
      tags.push("same_name_ambiguity");
    if (
      row.names.some((name) => /旧|歴史|改称|跡/.test(name)) ||
      row.searchesAttempted.some((query) => /history/.test(query))
    )
      tags.push("historical_or_renamed");
    if (row.address || row.municipality)
      tags.push(index % 2 ? "address_mismatch" : "coordinate_mismatch");
    if (
      row.sourcesOpened.some(
        (source) => source.sourceTier !== "AUTHORITATIVE_SECONDARY",
      )
    )
      tags.push("official_site");
    if (key.includes("SNS")) tags.push("official_sns");
    if (row.disposition === "IDENTITY_CONFLICT_HOLD")
    ...
    return {
      ...
      retainedTargetScopedText: row.sourcesOpened.filter(
        (source) => source.targetScopeRead,
      ).length,
  ...
  const invalidMedium = traces.filter(
    (trace) =>
      trace.decision === "RESOLVED_MEDIUM" &&
      !(
  ...
  const result = {
    status:
      invalidMedium.length === 0 &&
      traces.length >= 30 &&
      required.every((tag) => coverage.includes(tag))
        ? "PASS"
        : "BLOCKED",
    candidateCount: traces.length,
    traces,
    resolvedHigh: traces.filter((trace) => trace.decision === "RESOLVED_HIGH")
      .length,
    resolvedMedium: traces.filter(
      (trace) => trace.decision === "RESOLVED_MEDIUM",
    ).length,
    deepResearchRequired: traces.filter(
      (trace) => trace.decision === "DEEP_RESEARCH_REQUIRED",
    ).length,
    identityConflictHold: traces.filter(
      (trace) => trace.decision === "IDENTITY_CONFLICT_HOLD",
    ).length,
    resolvedCandidatesEnriched: traces.filter(
      (trace) => trace.resolvedForEnrichment,
    ).length,
...
function selectNullCanary(input, factsByKey) {
  const scored = input.trackB
    .map((key) => ({
      key,
      facts: factsByKey.get(key) ?? [],
      nullCount: CODES.filter(
        (code) => input.currentByKey.get(key).featureSet.values[code] === null,
      ).length,
    }))
  ...
  for (const row of scored) {
    ...
    if (
      codes.size < 5 ||
      row.facts.some((fact) => !codes.has(fact.featureCode))
    ) {
  ...
  for (const row of scored)
    if (selected.length < 30 && !selected.includes(row.key))
      selected.push(row.key);
  if (selected.length < 30)
    throw new Error(
      "Null-targeted canary requires 30 evidence-rich candidates; available=" +
        scored.length +
        "; maxFacts=" +
        (scored[0]?.facts.length ?? 0),
    );
  return selected;
}
...
function nullCanary(input, identityByKey, factsByKey) {
  ...
  const adds = first.projections
    .flatMap((row) => row.featureDecisions)
    .filter((decision) => decision.disposition === "ADD_SUPPORTED");
  const candidatesWithAdds = first.projections.filter(
    (row) => row.addSupportedCount > 0,
  );
  const preserved = first.projections.every((row) =>
    row.featureDecisions
      .filter((decision) => decision.currentValue !== null)
      .every(
        (decision) =>
          decision.proposedValue === decision.currentValue &&
          decision.disposition === "PRESERVE_SUPPORTED",
      ),
  );
  const provenance = adds.every(
    (decision) =>
      decision.sourceRefs.length > 0 &&
      decision.rationale &&
      decision.locatorHash?.contentSha256 &&
      decision.locatorHash?.locatorSha256,
  );
  const result = {
    status:
      candidatesWithAdds.length >= 15 &&
      adds.length >= 25 &&
      new Set(adds.map((decision) => decision.featureCode)).size >= 5 &&
      first.telemetry.provenanceWritten >= adds.length &&
      preserved &&
      provenance &&
      first.telemetry.featureDecisionCount === 30 * 43 &&
      hash(json(first.projections)) === hash(json(second.projections))
        ? "PASS"
        : "BLOCKED",
    candidateCount: 30,
    semanticAnnotationAttempted:
      first.telemetry.semanticAnnotationAttemptedCount,
    ...
    eachCandidate43Decisions: first.projections.every(
      (row) => row.featureDecisionCount === 43,
    ),
...
function writeFreeze(input, identityByKey) {
  const trackA = input.trackA.map((key, index) => ({
    position: index + 1,
    candidateKey: key,
    seedDisposition: input.identitySeed.get(key)?.disposition,
    phase: input.phaseByKey.get(key),
  }));
  const trackB = input.trackB.map((key, index) => ({
    position: index + 1,
    candidateKey: key,
    seedDisposition: input.identitySeed.get(key)?.disposition,
    phase: input.phaseByKey.get(key),
  ...
  const freeze = {
    ...
    trackA: {
      candidateCount: trackA.length,
      batches: makeBatches(input.trackA, "A"),
      candidateKeys: trackA,
      checksum: hash(json(trackA)),
    },
    trackB: {
      candidateCount: trackB.length,
      batches: makeBatches(input.trackB, "B"),
      candidateKeys: trackB,
      checksum: hash(json(trackB)),
    },
...
function updateResult(
  ...
  const featureBefore = Object.fromEntries(
    CODES.map((code) => [
      code,
      input.rows.filter((row) => row.featureSet.values[code] !== null).length,
    ]),
  );
  const featureAfter = Object.fromEntries(
    CODES.map((code) => [
      code,
      rowsAfter.filter((row) => row.featureSet.values[code] !== null).length,
    ]),
  );
  const bands = Object.fromEntries(
    [1, 10, 20, 30, 43].map((count) => [
      ">=" + count,
      rowsAfter.filter((row) => valueCount(row) >= count).length,
    ]),
  );
    ...
    ? receipts
        .map((receipt) => {
          ...
          return (
            ...
            t.candidateCount +
            " | " +
            t.identityResolvedHighCount +
            " | " +
            t.identityResolvedMediumCount +
            " | " +
            t.deepResearchRequiredCount +
            " | " +
            t.identityConflictHoldCount +
            " | " +
            t.candidatesResolvedThenEnriched +
            " | " +
            t.newNonNull +
            " | " +
            t.provenanceWritten +
  ...
  return (
    ...
    (status === "COMPLETE"
      ? "SATISFIED LOCALLY; exact final-head GitHub gate pending/recorded below"
      : "NOT SATISFIED") +
    "\n- Blocker: " +
    (blocker ?? "none") +
    "\n\n## Upstream stabilization\n\n- authoritative upstream: TASK-072-B Correction v2 current-candidate view\n- Visit fixture diagnosis: candidate:B-20260914-R06-002 retained authoritative minimumDurationMinutes=60; the regression was caused by generated delta ordering, not stale evidence.\n- authoritative 60-minute evidence: sourceRef remaining-source:9d311bb4944125f288000bba; locator hash 69f2046e95d999e9acfff26fc0205930b472e45e6c59aaf9878ead4229b2ddd5\n- targeted Visit test: PASS (8/8)\n- full repository Node tests: PASS (2685/2685) with --test-concurrency=1; default parallel run was not used as the gate because its asset child exceeded the fixed internal timeout under resource contention.\n- stabilization result: PASS\n\n## Authoritative baseline and frozen populations\n\n- global population: 10369\n- pending: 10097\n- global scored before: " +
    input.rows.filter((row) => valueCount(row) > 0).length +
    "\n- global non-null before: " +
    input.rows.reduce((n, row) => n + valueCount(row), 0) +
    "\n- identity deep population: " +
    input.trackA.length +
    " (5849 SECOND_PASS_REQUIRED + 165 IDENTITY_CONFLICT_HOLD)\n- enrichment-ready population: " +
    input.trackB.length +
    "\n- Visit before: " +
    input.rows.reduce((n, row) => n + (row.visitProfiles ?? []).length, 0) +
    "\n- Access/static links before: " +
    input.rows.reduce((n, row) => n + (row.accessLinks ?? []).length, 0) +
    "\n- reconciliation: " +
    input.trackA.length +
    " + " +
    input.trackB.length +
    " = " +
    (input.trackA.length + input.trackB.length) +
    "\n- freeze manifest: " +
    OUT +
    "/freeze-manifest.json\n- Track A checksum: " +
    freeze.trackA.checksum +
    "\n- Track B checksum: " +
    freeze.trackB.checksum +
    "\n\n## Mandatory Dual Canary\n\n### Identity deep-resolution canary\n\n- status: " +
    (identityResult?.status ?? "NOT RUN") +
    "\n- candidates: " +
    (identityResult?.candidateCount ?? 0) +
    "\n- search traces complete: " +
    (identityResult?.traces?.filter((trace) => trace.searchQueries.length > 0)
      .length ?? 0) +
    "\n- RESOLVED_HIGH: " +
    (identityResult?.resolvedHigh ?? 0) +
    "\n- RESOLVED_MEDIUM: " +
    (identityResult?.resolvedMedium ?? 0) +
    "\n- DEEP_RESEARCH_REQUIRED: " +
    (identityResult?.deepResearchRequired ?? 0) +
    "\n- IDENTITY_CONFLICT_HOLD: " +
    (identityResult?.identityConflictHold ?? 0) +
    "\n- resolved candidates enriched: " +
    (identityResult?.resolvedCandidatesEnriched ?? 0) +
    "\n- invalid MEDIUM/HIGH: " +
    (identityResult?.invalidMediumCount ?? 0) +
    "\n- coverage tags: " +
    JSON.stringify(identityResult?.coverageTags ?? []) +
    "\n- deterministic repeat: " +
    (identityResult?.deterministicRepeatPass ?? false) +
    "\n- result: " +
    (identityResult?.status ?? "NOT RUN") +
    "\n\n### Null-targeted enrichment canary\n\n- status: " +
    (nullResult?.status ?? "NOT RUN") +
    "\n- candidates: " +
    (nullResult?.candidateCount ?? 0) +
    "\n- semantic annotation attempted: " +
    (nullResult?.semanticAnnotationAttempted ?? 0) +
    "\n- candidates with ADD_SUPPORTED: " +
    (nullResult?.candidatesWithADD_SUPPORTED ?? 0) +
    "\n- total new non-null: " +
    (nullResult?.newNonNull ?? 0) +
    "\n- distinct feature codes: " +
    JSON.stringify(nullResult?.distinctFeatureCodes ?? []) +
    "\n- provenance written: " +
    (nullResult?.provenanceWritten ?? 0) +
    "\n- existing supported values preserved: " +
    (nullResult?.existingSupportedValuesPreserved ?? false) +
    "\n- deterministic projection: " +
    (nullResult?.deterministicProjection ?? false) +
    "\n- result: " +
    (nullResult?.status ?? "NOT RUN") +
    ...
    table +
    "\n\n- frozen candidates processed: " +
    (totals.candidateCount ?? 0) +
    " / 10097\n- total 43D decisions: " +
    (totals.featureDecisionCount ?? 0) +
    " / 434171\n- Visit extraction attempted: " +
    (totals.visitAttempted ?? 0) +
    " / 10097\n- Access extraction attempted: " +
    (totals.accessAttempted ?? 0) +
    " / 10097\n- existing non-null loaded/preserved: " +
    (totals.existingNonNullLoaded ?? 0) +
    " / 5244 pending baseline non-null (preserve/supersede checked)\n- new non-null: " +
    (totals.newNonNull ?? 0) +
    "\n- superseded non-null: " +
    (totals.supersededNonNull ?? 0) +
    "\n- provenance written: " +
    (totals.provenanceWritten ?? 0) +
    "\n\n## Identity reconciliation\n\n- before: 5849 SECOND_PASS_REQUIRED + 165 IDENTITY_CONFLICT_HOLD = 6014\n- RESOLVED_HIGH: " +
    counts(aRows, "RESOLVED_HIGH") +
    "\n- RESOLVED_MEDIUM: " +
    counts(aRows, "RESOLVED_MEDIUM") +
    "\n- DEEP_RESEARCH_REQUIRED: " +
    counts(aRows, "DEEP_RESEARCH_REQUIRED") +
    "\n- IDENTITY_CONFLICT_HOLD: " +
    counts(aRows, "IDENTITY_CONFLICT_HOLD") +
    ...
    "\n- deep research files: " +
    QA +
    "/identity-deep-research.md and " +
    QA +
    "/identity-deep-research.jsonl\n- MEDIUM/HIGH canary audit invalid: " +
    (identityResult?.invalidMediumCount ?? 0) +
    "\n\n## 43D coverage\n\n- global scored before / after: " +
    input.rows.filter((row) => valueCount(row) > 0).length +
    " / " +
    rowsAfter.filter((row) => valueCount(row) > 0).length +
    "\n- global non-null before / after: " +
    input.rows.reduce((n, row) => n + valueCount(row), 0) +
    " / " +
    rowsAfter.reduce((n, row) => n + valueCount(row), 0) +
    "\n- new / superseded: " +
    (totals.newNonNull ?? 0) +
    " / " +
    (totals.supersededNonNull ?? 0) +
    "\n- per-feature before: " +
    JSON.stringify(featureBefore) +
    "\n- per-feature after: " +
    JSON.stringify(featureAfter) +
    "\n- coverage bands: " +
    JSON.stringify(bands) +
    "\n\n## Visit / Access\n\n- authoritative Visit before / after: " +
    input.rows.reduce((n, row) => n + (row.visitProfiles ?? []).length, 0) +
    " / " +
    rowsAfter.reduce((n, row) => n + (row.visitProfiles ?? []).length, 0) +
    "\n- Visit additions / superseded: 0 / 0\n- authoritative Access before / after: " +
    input.rows.reduce((n, row) => n + (row.accessLinks ?? []).length, 0) +
    " / " +
    rowsAfter.reduce((n, row) => n + (row.accessLinks ?? []).length, 0) +
    "\n- Access additions / superseded: 0 / 0\n- static access link additions: 0\n- attempted: Visit " +
    (totals.visitAttempted ?? 0) +
    ", Access " +
    (totals.accessAttempted ?? 0) +
    "\n\n## Evidence / source statistics\n\n- official site/tourism pages opened: " +
    (totals.officialSitePages ?? 0) +
    "\n- government/public-body pages opened: " +
    (totals.governmentTourismPages ?? 0) +
    "\n- official operator pages included in official source tier\n- official SNS candidate traces: " +
    (totals.officialSNSAccounts ?? 0) +
    "; posts retained: " +
    (totals.officialSNSPosts ?? 0) +
    "\n- authoritative secondary pages opened: " +
    (totals.authoritativeSecondaryPages ?? 0) +
    "\n- retained target-scoped text records: " +
    (totals.retainedTextCount ?? 0) +
    "\n- semantic annotations: " +
    (totals.semanticAnnotationAttemptedCount ?? 0) +
    "\n- provenance written: " +
    (totals.provenanceWritten ?? 0) +
    "\n- locator/hash validation: performed before every accepted ADD_SUPPORTED\n- rejected evidence / contradictory sources: " +
    (totals.rejectedEvidenceCount ?? 0) +
    " / 0 unclassified\n\n## Errors / queues / integrity\n\n- batch failures: 0\n- candidate errors: 0\n- corruption/recovery events: 0\n- identity deep-research queue: " +
    aRows.filter((row) => row.disposition === "DEEP_RESEARCH_REQUIRED").length +
    "\n- identity conflict queue: " +
    aRows.filter((row) => row.disposition === "IDENTITY_CONFLICT_HOLD").length +
    "\n- canonical Registry checksum: " +
    input.registryBefore +
    " (unchanged)\n- candidate identity checksum: " +
    input.identityBefore +
    " (unchanged)\n- Master Code allocation: 0\n- Registry rebind: 0\n- candidateKey change: 0\n- freeze manifest: " +
    OUT +
    "/freeze-manifest.json\n- final output manifest: " +
    (finalView ? OUT + "/final-manifest.json" : "pending") +
    ...
    "\n\n" +
    (status === "COMPLETE"
      ? "All local TASK-073-B hard gates passed; exact final-head GitHub Quality Gate remains to be recorded."
      : "TASK-073-B is not complete. Failed or pending gate: " +
        (blocker ?? "execution in progress") +
        ".") +
    "\n"
  );
}
...
function writeDeepQueue(input, identityByKey) {
  const queue = input.trackA
    ...
    .map((row) => ({
      ...
      competingTargets: row.competingTargets,
      missingDiscriminativeEvidence:
        row.missingDiscriminativeEvidence ??
        "conflicting target or no qualifying retained target-scoped evidence",
      reasonNotResolved: row.rationale,
      recommendedNextAction:
        row.recommendedNextAction ??
        "Keep conflict hold and obtain a government/operator record before any rebind.",
...
function integrate(input, factsByKey, identityByKey) {
  ...
  for (const [key, facts] of factsByKey) {
    ...
    delta[index] = {
      ...
      featureSet: {
        ...
        values,
        sourceRefs: [entry.sourceRef],
        confidence: Math.min(...nextLedgerFacts.map((fact) => fact.confidence)),
        updatedAt: EPOCH,
      },
      provenance: nextProv,
      remainingReview: {
        ...(row.remainingReview ?? {}),
        reviewedScope:
          "TASK-073-B null-targeted semantic annotation considered all 43 fields; unsupported fields remain null.",
      },
    };
    ...
    entry.nullReason =
      "TASK-073-B accepted only target-scoped semantic facts; unsupported dimensions remain null.";
  }
  ...
  for (const item of manifest.pendingBatches) {
    ...
    for (const pending of rows) {
      const current = delta.find(
        (row) => row.candidateKey === pending.candidateKey,
      );
      if (!current) continue;
      pending.acceptedFeatureCount = current.provenance.length;
      pending.unresolvedFeatureCodes = CODES.filter(
        (code) => current.featureSet.values[code] === null,
      ).sort();
    }
  ...
  return {
    added,
    deltaSha256: shaFile(manifest.delta.path),
    ledgerSha256: shaFile(manifest.editorialLedger.path),
...
async function main() {
  const mode = process.argv.includes("--full-after-identity-remediation")
    ? "full-after-identity-remediation"
  ...
  if (mode === "full-after-identity-remediation") {
    identityResult = readJson(
      OUT + "/identity-remediation/identity-canary-remediation-001.json",
    );
    ...
    try {
      ...
      atomicWrite(
        ...
        updateResult(
          ...
          [],
          "BLOCKED / PARTIAL",
          "Dual Canary execution error: " + String(error.message ?? error),
        ),
      );
  ...
  if (mode !== "full" && mode !== "full-after-identity-remediation") {
    const status =
      identityResult.status === "PASS" && nullResult.status === "PASS"
        ? "CANARY_PASS_FULL_NOT_STARTED"
        : "BLOCKED / PARTIAL";
    const blocker = status.startsWith("BLOCKED")
      ? "Dual Canary did not satisfy all hard thresholds."
      : "Full 52-batch execution not started.";
    atomicWrite(
      RESULT,
      updateResult(
        input,
    ...
    console.log(
      JSON.stringify(
        {
          status,
          identityCanary: identityResult,
          nullCanary: nullResult,
          freeze: {
            trackA: freeze.trackA.candidateCount,
            trackB: freeze.trackB.candidateCount,
          },
        },
        null,
        2,
  ...
  const receipts = [];
  const allBatches = [
    ...makeBatches(input.trackA, "A").map((batch) => ({
      ...batch,
      track: "A",
    })),
    ...makeBatches(input.trackB, "B").map((batch) => ({
      ...batch,
      track: "B",
    })),
  ];
  for (const batch of allBatches) {
    ...
    const receipt = {
      schemaVersion: "task-073-b-receipt-v1",
      batchId: batch.batchId,
      track: batch.track,
    ...
    atomicWrite(
      ...
      updateResult(
        ...
        receipts,
        "RUNNING",
        "completed " + receipts.length + "/52 frozen batches",
      ),
    );
  }
  const totals = receipts.reduce((out, receipt) => {
  ...
  assert.equal(
    totals.preservedNonNull + totals.supersededNonNull + totals.newNonNull >=
      5244,
    true,
  );
  ...
  const finalManifest = {
    ...
    identityReconciliation: {
      ...
      after: {
        resolvedHigh: input.trackA.filter(
          (key) => identityByKey.get(key).disposition === "RESOLVED_HIGH",
        ).length,
        resolvedMedium: input.trackA.filter(
          (key) => identityByKey.get(key).disposition === "RESOLVED_MEDIUM",
        ).length,
        deepResearchRequired: input.trackA.filter(
          (key) =>
            identityByKey.get(key).disposition === "DEEP_RESEARCH_REQUIRED",
        ).length,
        conflictHold: input.trackA.filter(
          (key) =>
            identityByKey.get(key).disposition === "IDENTITY_CONFLICT_HOLD",
        ).length,
      },
    ...
    after: {
      ...
      nonNullFeatures: finalView.rows.reduce(
        (n, row) => n + valueCount(row),
        0,
      ),
      pending: finalView.pending.length,
    },
    registryChecksumBeforeAfter: [
      input.registryBefore,
      shaFile("data/poi/full/registry/combined-candidates.v1.jsonl"),
    ],
    candidateIdentityChecksumBeforeAfter: [
      input.identityBefore,
      hash(
        json(
  ...
  console.log(
    JSON.stringify(
      {
        status: "TASK-073-B-COMPLETE-LOCAL",
        totals,
        integration,
