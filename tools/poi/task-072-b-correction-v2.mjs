...
function integrateCanonical(input, facts, identities) {
  ...
  for (const [key, byCode] of factsByKey) {
    ...
    const canonicalSourceRef = e.sourceRef; assert(canonicalSourceRef);
    ...
    e.identityAssessment = { status: "TARGET_CONFIRMED", method: "Correction v2 semantic evidence annotation", rationale: identity.rationale }; e.features = accepted.map((fact) => ({ code: fact.featureCode, value: fact.suggestedValue, confidence: fact.confidence, annotationMethod: "editorial_calibration", reason: fact.reason, locator: fact.locator })); e.unassessedFieldsRemainNull = true; e.assessmentOutcome = "CORRECTION_V2_ACCEPTED_FACTS"; e.nullReason = "Correction v2 accepted only explicitly evidenced fields; all other dimensions remain null.";
  }
  delta.sort((a, b) => a.candidateKey.localeCompare(b.candidateKey)); ledger.entries.sort((a, b) => a.position - b.position);
...
async function main() {
  ...
  if (mode !== "full") return;
  const canonicalFacts = facts.filter((fact) => { const entry = input.ledger.entries.find((item) => item.candidateKey === fact.candidateKey); const boundary = entry?.targetBoundary; const insideBoundary = !boundary || (boundary.offset <= fact.locator.offset && fact.locator.offset + fact.locator.length <= boundary.offset + boundary.length); return Boolean(entry?.sourceRef && entry?.source && insideBoundary); }); const factsByKey = factMap(canonicalFacts); const receipts = [];
  ...
  const integration = integrateCanonical(input, canonicalFacts, identities); const after = readCurrentCandidateRows(); const valueCount = (r) => Object.values(r.featureSet.values).filter((v) => v !== null).length; assert(after.rows.filter((r) => valueCount(r) > 0).length >= 2510); assert(after.rows.reduce((n, r) => n + valueCount(r), 0) >= 6104);
