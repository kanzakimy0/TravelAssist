...
import { providerAvailability } from "../../lib/auth/provider-availability";
...
export async function AuthPage({
  ...
  if (
    signedIn &&
    (kind === "login" || (kind === "register" && query.confirmed !== "1"))
  )
    redirect(returnTo);
  if (kind === "register" && signedIn && query.confirmed === "1")
    return (
...
export function AuthUnavailable() {
  return (
    <section className={styles.unavailable}>
      <h1>暂时无法确认登录状态</h1>
      <p role="alert">为保护你的账户，个人中心暂不展示。请稍后重试。</p>
      <Link href={authHref("/login", "/personal-center")}>返回登录</Link>
    </section>
  );
}
