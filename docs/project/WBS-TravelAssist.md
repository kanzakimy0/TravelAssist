...
| 7.9    | 推荐打分 v1                   | A      | P0     | 5.14,7.4 | 待人工盲审（TASK-038-A / #299 + TASK-039/040 v2 质量修正；候选未冻结） |
...
- Primary / Near-score 仅从双方均适配场景的 POI 中出题；新增 `NEITHER_SUITABLE`，并与
  `INSUFFICIENT_INFO` 分离。
- 首轮 R1/R2 仅保留脱敏 hash 与聚合质量指标，禁止进入 Human Gold 或 candidate-0457 评估；
  v2 中文工作簿为空白，需重新进行独立人工评审。
- 候选参数未修改，WBS 7.9 继续保持 `待人工盲审`。
