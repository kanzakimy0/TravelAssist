...
| TASK-008.2 | 1.5 / 1.6 / 1.7 / 1.14；4.1（Planner 纯视觉精修） | A | 待审查（Partially Completed；参考图及 v0.3 范围待确认） | #73 | `docs/tasks/TASK-008.2-a-planner-visual-fidelity-polish.md` | `feature/a-planner-visual-fidelity-polish` | `7e8db2a`（实现），`45c3c3f`（同步文档） | 创建后回填；必须 Draft，不自动合并 |
