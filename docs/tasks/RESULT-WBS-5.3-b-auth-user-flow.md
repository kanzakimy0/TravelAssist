...
待审查 — the user-authorized callback error/confirmation feedback fix below has passed real Local regression. Final onsite user acceptance remains pending. Missing original-browser PKCE state still correctly prevents authentication; no cross-browser bypass or claim that the original user's cookie-loss cause was conclusively identified. Historical sections below describe their execution-time status, not the latest fix. Full-repository format baseline remains separately reported; no subsequent task started.
