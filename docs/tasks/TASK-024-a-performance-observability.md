...
- Depends On: 2.11 Error / Logging规范；执行时核验实际实现
...
仓库原有绝对指标不达参考值时如实登记与建议；只允许本Task范围内小修，不借机重做视觉或资产体系。质量报告必须区分预算通过、已知基线欠债与未测项目。
...
- 独立观测实现与测试，按实际目录边界命名，不新增第二套公共Trip模型。
- `tools/qa/` 下可复跑性能采集/预算脚本和 `tests/task-024-*.test.mjs`。
- `docs/quality/performance-observability.md`：指标定义、预算、错误级别、sink配置、关闭/回滚与排查方法。
- `docs/qa/task-024/` 下脱敏基线和对比报告；含commit和测试环境，不提交敏感HAR/trace。
- Result / Issue / Master WBS / Draft PR，同步当前实现状态。

按共同规则跑全仓检查；接脚本到CI时与025协调，不另建互相冲突的workflow。#231未合并时不复制其扫描器，也不把本Taskcanary测试等同9.10已完成。
