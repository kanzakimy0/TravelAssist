...
**Partial / 阻塞（页面集成缺口）；不是 Completed，也不是 Ready for review。**

服务器持久化子集已实现并通过本地真实验证，拟以 Draft PR 保存供审查。实际 Step 页面尚未切换到服务器 autosave/resume；原有本地草稿未被替换。后续不自动开始 5.14 / 5.19 / 8.5 / Auth Core。
...
- 真实本地 Auth API 验证无效 token/伪造 owner 拒绝，同 key 并发创建仅一草稿，同版本并发更新仅一个成功。
...
- 全仓测试：最终复跑 **621 PASS**（原 612 + 本次 9），无失败/跳过。
...
- `src/server/preferences/{repository,service}.ts`
...
- 未修改 Planner/Detail/Personal Center/Start 的业务 UI、A 主 Trip 契约、环境文件或 package-lock。

## Known Limitations

1. **整体不是完整实现**：Step 页面与 Personal Center 仍保持现有浏览器/内存存储。新 API 和 autosave 控制器尚未被这些页面调用。
2. Auth Core 尚无身份交接/用户切换生命周期入口；不得自行建立匿名账户或假登录来凑完成。
...
- 先审查 Draft 中的数据库与服务子集；WBS 5.11/5.16 为子集待审查，5.18 为部分实现且页面接入阻塞，不写最终完成。
- 明确 Auth 身份交接、未登录/登录/换用户策略及 Step 金额上下文，完成剩余 typed mapping，再接入实际 autosave/resume 和浏览器跨设备验收。
