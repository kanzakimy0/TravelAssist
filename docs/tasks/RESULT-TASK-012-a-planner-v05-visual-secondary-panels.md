...
- lint: Passed。
- typecheck: Passed。
- tests: `npm run test --if-present` 无现有 test script；额外运行全部 `tests/*.test.mjs`，103/103 Passed（新增 16 项）。原 TASK-010 静态导航断言更新为新的 WorkspaceHeader 入口，保留真实首页与个人中心链接验证。
- format: Changed files Passed；全仓 format:check 仍有 24 个未修改文件的既有告警。干净基线复验为 25 个，本 Task 修复自身 Task 文档的一个告警，未批量改无关文档。
...
- 103 项 tests、六尺寸 [Mapbox](../qa/TASK-012/qa-mapbox.json) / [fallback](../qa/TASK-012/qa-fallback.json)、[交互验收](../qa/TASK-012/interaction-qa.json)、[TASK-011 Detail 回归](../qa/TASK-012/detail-regression/report.json)。
...
2. 全仓 24 个既有格式告警仍在，详见 [格式基线](../qa/TASK-012/format-baseline.md)；不属于本 Task 新增问题。
