...
| Preference / Companion   | Preference：src/shared/contracts/preferences/index.ts（唯一 core 提升，旧 B 路径兼容）；B / 5.14；Companion 原内部模型保持 | Preference GET-only lib/preferences/client.ts 与 server/preferences/public-read.ts；[交接记录](../contracts/preference-read-v1-handoff.md) | TASK-046-B Producer / automated consumer QA 通过，A 人工 review Pending；4.18/5.17 未启动；PR #221 仍为历史候选 |
